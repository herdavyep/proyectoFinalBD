...
require_once './calidad_const.php';
...
function art_datagrid_display($field_names, $page_size, $current_page, $quick_search, $navtype, $category, $showtotalrec=0, $showpagesize=0) {	
    ...
    $clr = art_request("clr_calidad", "");
    $clr_adv_session = art_request("clr_calidad_adv_session", "");

    if (strtolower($clr) == "t"){
        art_clear_session("calidad_sort1");	
        art_clear_session("calidad_sort2");	
        art_clear_session("calidad_sort3");	
        art_clear_session("calidad_sort4");	
        art_clear_session("calidad_sort5");	
        $clr = "";
	  }

    if (strtolower($clr_adv_session) == "y") {
        $clr_adv_session = "";
        art_clear_session("calidad_search");
        art_clear_session("calidad_page");
        art_clear_session("calidad_quick_search");
        art_clear_session("calidad_category");
        $quick_search = "";
        $category = "";
	  }

    art_assign_session("calidad_sort1", "");
    art_assign_session("calidad_sort2", "");
    art_assign_session("calidad_sort3", "");
    art_assign_session("calidad_sort4", "");
    art_assign_session("calidad_sort5", "");
    art_assign_session("calidad_page_size", 20);
    art_assign_session("calidad_page", "1");
    $sql_array  = art_split_sql(" select * from `calidad`"); 
    $sql_start = $sql_array[0]; 
    $sql_orderby = $sql_array[1]; 
    $sort1 = art_session("calidad_sort1", "");
    $sort2 = art_session("calidad_sort2", "");
    $sort3 = art_session("calidad_sort3", "");
    $sort4 = art_session("calidad_sort4", "");
    $sort5 = art_session("calidad_sort5", "");
    ...
    for ($i=1; $i<=5; $i++){
        $sorting = "";
        $sort_order = "";
        if (art_session("calidad_sort".$i, "") == "1"){
            $sorting = "&calidad_sort". $i . "=" . "2";
            $sort_order = "ASC";
        } else if (art_session("calidad_sort".$i, "") == "2"){
            $sorting = "&calidad_sort". $i . "=" . "1";
            $sort_order = "DESC";
        } else {
            $sorting = "&calidad_sort". $i . "=" . "1";
            $sort_order = "";
        }
        $qrystr[$i] = "calidad_ajax.php?clr_calidad=t".$sorting;
    ...
    $advsearch_sql = art_session("calidad_search","");
    if ($advsearch_sql != "") {
    ...
    art_sitemenu_display("calidad", "defaulttheme");
    print "</td>\n";
    ...
    print "<a href=\"" . art_gen_url("calidad_ajax.php?clr_calidad_adv_session=y", 1). "\"" . " title=\"Show All\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/show_all.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . art_gen_url("calidad_ajax.php?clr_calidad=t", 1). "\"" . " title=\"Clear Sort\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/clear_sort.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . "./calidad_search.php". "\"" . " title=\"Advance Search\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/adv_search.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . "./calidad_add.php". "\"" . " title=\"Add New Record\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/add.gif\" border=\"0\" /></a>";
    print "&nbsp;";
    print "                                </td>\n";
    print "                            </tr>\n";
    print "                        </table>\n";
    ...
    if ( ($numrows > 0) && ($result) ) {
        ...
        while ($row = mysql_fetch_array($result)){ 
            ...
            print "<a href=\"" . "./calidad_edit.php". "?pm_id=" . art_rowdata_byname($row, $field_names, "calidad.id"). "\"" . " title=\"Edit\" target=\"_self\" class=\"gridBodyLink\">";
            ...
            print "</td>\n";
            print "<td align=\"center\" >";
            print "<a href=\"" . "./calidad_delete.php". "?pm_id=" . art_rowdata_byname($row, $field_names, "calidad.id"). "\"" . " title=\"Delete\" target=\"_self\" class=\"gridBodyLink\">";
    ...
    if ( ($numrows > 0) && ($result) ){
        ...
        if ($navtype != ""){
            $pagename = "calidad";
            art_show_navigator($pagename, $navtype, $numrows, $page_size, $current_page, "calidad_ajax.php", 1, 0, $use_ajax, $folder_button_images);
        }
    } else {
    ...
    for ($i=1; $i <= $pagerecords; $i++){
        ...
        if ($del_id != ""){
            ...
            if ($j==0){
                ...
                $sqlDel = "DELETE FROM `calidad` WHERE ";
                $sqlDel .= "calidad.id = " .  art_quote_intval($artv_pm_id);

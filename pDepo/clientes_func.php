...
require_once './clientes_const.php';
...
function art_datagrid_display($field_names, $page_size, $current_page, $quick_search, $navtype, $category, $showtotalrec=0, $showpagesize=0) {	
    ...
    $clr = art_request("clr_clientes", "");
    $clr_adv_session = art_request("clr_clientes_adv_session", "");

    if (strtolower($clr) == "t"){
        art_clear_session("clientes_sort1");	
        art_clear_session("clientes_sort2");	
        art_clear_session("clientes_sort3");	
        art_clear_session("clientes_sort4");	
        art_clear_session("clientes_sort5");	
        art_clear_session("clientes_sort6");	
        $clr = "";
	  }

    if (strtolower($clr_adv_session) == "y") {
        $clr_adv_session = "";
        art_clear_session("clientes_search");
        art_clear_session("clientes_page");
        art_clear_session("clientes_quick_search");
        art_clear_session("clientes_category");
        $quick_search = "";
        $category = "";
	  }

    art_assign_session("clientes_sort1", "");
    art_assign_session("clientes_sort2", "");
    art_assign_session("clientes_sort3", "");
    art_assign_session("clientes_sort4", "");
    art_assign_session("clientes_sort5", "");
    art_assign_session("clientes_sort6", "");
    art_assign_session("clientes_page_size", 20);
    art_assign_session("clientes_page", "1");
    $sql_array  = art_split_sql(" select * from `clientes`"); 
    $sql_start = $sql_array[0]; 
    $sql_orderby = $sql_array[1]; 
    $sort1 = art_session("clientes_sort1", "");
    $sort2 = art_session("clientes_sort2", "");
    $sort3 = art_session("clientes_sort3", "");
    $sort4 = art_session("clientes_sort4", "");
    $sort5 = art_session("clientes_sort5", "");
    $sort6 = art_session("clientes_sort6", "");
    ...
    for ($i=1; $i<=6; $i++){
        $sorting = "";
        $sort_order = "";
        if (art_session("clientes_sort".$i, "") == "1"){
            $sorting = "&clientes_sort". $i . "=" . "2";
            $sort_order = "ASC";
        } else if (art_session("clientes_sort".$i, "") == "2"){
            $sorting = "&clientes_sort". $i . "=" . "1";
            $sort_order = "DESC";
        } else {
            $sorting = "&clientes_sort". $i . "=" . "1";
            $sort_order = "";
        }
        $qrystr[$i] = "clientes_ajax.php?clr_clientes=t".$sorting;
    ...
    $advsearch_sql = art_session("clientes_search","");
    if ($advsearch_sql != "") {
    ...
    art_sitemenu_display("clientes", "defaulttheme");
    print "</td>\n";
    ...
    print "<a href=\"" . art_gen_url("clientes_ajax.php?clr_clientes_adv_session=y", 1). "\"" . " title=\"Show All\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/show_all.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . art_gen_url("clientes_ajax.php?clr_clientes=t", 1). "\"" . " title=\"Clear Sort\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/clear_sort.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . "./clientes_search.php". "\"" . " title=\"Advance Search\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/adv_search.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . "./clientes_add.php". "\"" . " title=\"Add New Record\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/add.gif\" border=\"0\" /></a>";
    print "&nbsp;";
    print "                                </td>\n";
    print "                            </tr>\n";
    print "                        </table>\n";
    ...
    print "                    <td class=\"gridColumn\"  NOWRAP >\n";
    print "                        <div class=\"gridColumnText\">\n";
    print "                            <a href=\"" . art_gen_url($qrystr[4], 1) . "\" class=\"gridColumnLink\">Fecha Creacion</a>\n";
    if ($sort4 == "1") {
    ...
    if ( ($numrows > 0) && ($result) ) {
        ...
        while ($row = mysql_fetch_array($result)){ 
            ...
            print "<a href=\"" . "./clientes_edit.php". "?pm_id=" . art_rowdata_byname($row, $field_names, "clientes.id"). "\"" . " title=\"Edit\" target=\"_self\" class=\"gridBodyLink\">";
            ...
            print "</td>\n";
            print "<td align=\"center\" >";
            print "<a href=\"" . "./clientes_delete.php". "?pm_id=" . art_rowdata_byname($row, $field_names, "clientes.id"). "\"" . " title=\"Delete\" target=\"_self\" class=\"gridBodyLink\">";
    ...
    if ( ($numrows > 0) && ($result) ){
        ...
        if ($navtype != ""){
            $pagename = "clientes";
            art_show_navigator($pagename, $navtype, $numrows, $page_size, $current_page, "clientes_ajax.php", 1, 0, $use_ajax, $folder_button_images);
        }
    } else {
    ...
    for ($i=1; $i <= $pagerecords; $i++){
        ...
        if ($del_id != ""){
            ...
            if ($j==0){
                ...
                $sqlDel = "DELETE FROM `clientes` WHERE ";
                $sqlDel .= "clientes.id = " .  art_quote_intval($artv_pm_id);

...
require_once './temporal_const.php';
...
function art_datagrid_display($field_names, $page_size, $current_page, $quick_search, $navtype, $category, $showtotalrec=0, $showpagesize=0) {	
    ...
    $clr = art_request("clr_temporal", "");
    $clr_adv_session = art_request("clr_temporal_adv_session", "");

    if (strtolower($clr) == "t"){
        art_clear_session("temporal_sort1");	
        art_clear_session("temporal_sort2");	
        art_clear_session("temporal_sort3");	
        art_clear_session("temporal_sort4");	
        art_clear_session("temporal_sort5");	
        $clr = "";
	  }

    if (strtolower($clr_adv_session) == "y") {
        $clr_adv_session = "";
        art_clear_session("temporal_search");
        art_clear_session("temporal_page");
        art_clear_session("temporal_quick_search");
        art_clear_session("temporal_category");
        $quick_search = "";
        $category = "";
	  }

    art_assign_session("temporal_sort1", "");
    art_assign_session("temporal_sort2", "");
    art_assign_session("temporal_sort3", "");
    art_assign_session("temporal_sort4", "");
    art_assign_session("temporal_sort5", "");
    art_assign_session("temporal_page_size", 20);
    art_assign_session("temporal_page", "1");
    $sql_array  = art_split_sql(" select * from `temporal`"); 
    $sql_start = $sql_array[0]; 
    $sql_orderby = $sql_array[1]; 
    $sort1 = art_session("temporal_sort1", "");
    $sort2 = art_session("temporal_sort2", "");
    $sort3 = art_session("temporal_sort3", "");
    $sort4 = art_session("temporal_sort4", "");
    $sort5 = art_session("temporal_sort5", "");
    ...
    for ($i=1; $i<=5; $i++){
        $sorting = "";
        $sort_order = "";
        if (art_session("temporal_sort".$i, "") == "1"){
            $sorting = "&temporal_sort". $i . "=" . "2";
            $sort_order = "ASC";
        } else if (art_session("temporal_sort".$i, "") == "2"){
            $sorting = "&temporal_sort". $i . "=" . "1";
            $sort_order = "DESC";
        } else {
            $sorting = "&temporal_sort". $i . "=" . "1";
            $sort_order = "";
        }
        $qrystr[$i] = "temporal_ajax.php?clr_temporal=t".$sorting;
    ...
    $advsearch_sql = art_session("temporal_search","");
    if ($advsearch_sql != "") {
    ...
    art_sitemenu_display("temporal", "defaulttheme");
    print "</td>\n";
    ...
    print "<a href=\"" . art_gen_url("temporal_ajax.php?clr_temporal_adv_session=y", 1). "\"" . " title=\"Show All\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/show_all.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . art_gen_url("temporal_ajax.php?clr_temporal=t", 1). "\"" . " title=\"Clear Sort\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/clear_sort.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . "./temporal_search.php". "\"" . " title=\"Advance Search\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/adv_search.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . "./temporal_add.php". "\"" . " title=\"Add New Record\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/add.gif\" border=\"0\" /></a>";
    print "&nbsp;";
    print "                                </td>\n";
    print "                            </tr>\n";
    print "                        </table>\n";
    ...
    print "                    <td class=\"gridColumn\"  NOWRAP >\n";
    print "                        <div class=\"gridColumnText\">\n";
    print "                            <a href=\"" . art_gen_url($qrystr[2], 1) . "\" class=\"gridColumnLink\">Compra Id</a>\n";
    if ($sort2 == "1") {
    ...
    if ( ($numrows > 0) && ($result) ) {
        ...
        while ($row = mysql_fetch_array($result)){ 
            ...
            print "<a href=\"" . "./temporal_edit.php". "?pm_compra_id=" . art_rowdata_byname($row, $field_names, "temporal.compra_id"). "&pm_id=" . art_rowdata_byname($row, $field_names, "temporal.id"). "\"" . " title=\"Edit\" target=\"_self\" class=\"gridBodyLink\">";
            ...
            print "</td>\n";
            print "<td align=\"center\" >";
            print "<a href=\"" . "./temporal_delete.php". "?pm_compra_id=" . art_rowdata_byname($row, $field_names, "temporal.compra_id"). "&pm_id=" . art_rowdata_byname($row, $field_names, "temporal.id"). "\"" . " title=\"Delete\" target=\"_self\" class=\"gridBodyLink\">";
    ...
    if ( ($numrows > 0) && ($result) ){
        ...
        if ($navtype != ""){
            $pagename = "temporal";
            art_show_navigator($pagename, $navtype, $numrows, $page_size, $current_page, "temporal_ajax.php", 1, 0, $use_ajax, $folder_button_images);
        }
    } else {
...
function del_selected_items($pagerecords) {
    ...
    for ($i=1; $i <= $pagerecords; $i++){
        ...
        if ($del_id != ""){
            $artv_pm_compra_id = art_request("pm_compra_id" . $i, "");
            $artv_pm_id = art_request("pm_id" . $i, "");
            if ($j==0){

                $sqlDel = "DELETE FROM `temporal` WHERE ";
                $sqlDel .= "temporal.compra_id = " .  art_quote_intval($artv_pm_compra_id);
                $sqlDel .= " AND temporal.id = " .  art_quote_intval($artv_pm_id);
...
function art_groupdatagrid_display($field_names, $page_size, $current_page, $quick_search, $navtype, $category, $showtotalrec=0, $showpagesize=0) {	
    ...
    print "                    <td class=\"gridColumn\" ><div class=\"gridColumnText\">Compra Id</div></td>\n";

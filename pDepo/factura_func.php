...
require_once './factura_const.php';
...
function art_datagrid_display($field_names, $page_size, $current_page, $quick_search, $navtype, $category, $showtotalrec=0, $showpagesize=0) {	
    ...
    $clr = art_request("clr_factura", "");
    $clr_adv_session = art_request("clr_factura_adv_session", "");

    if (strtolower($clr) == "t"){
        art_clear_session("factura_sort1");	
        art_clear_session("factura_sort2");	
        art_clear_session("factura_sort3");	
        art_clear_session("factura_sort4");	
        art_clear_session("factura_sort5");	
        art_clear_session("factura_sort6");	
        art_clear_session("factura_sort7");	
        art_clear_session("factura_sort8");	
        $clr = "";
	  }

    if (strtolower($clr_adv_session) == "y") {
        $clr_adv_session = "";
        art_clear_session("factura_search");
        art_clear_session("factura_page");
        art_clear_session("factura_quick_search");
        art_clear_session("factura_category");
        $quick_search = "";
        $category = "";
	  }

    art_assign_session("factura_sort1", "");
    art_assign_session("factura_sort2", "");
    art_assign_session("factura_sort3", "");
    art_assign_session("factura_sort4", "");
    art_assign_session("factura_sort5", "");
    art_assign_session("factura_sort6", "");
    art_assign_session("factura_sort7", "");
    art_assign_session("factura_sort8", "");
    art_assign_session("factura_page_size", 20);
    art_assign_session("factura_page", "1");
    $sql_array  = art_split_sql(" select * from `factura`"); 
    $sql_start = $sql_array[0]; 
    $sql_orderby = $sql_array[1]; 
    $sort1 = art_session("factura_sort1", "");
    $sort2 = art_session("factura_sort2", "");
    $sort3 = art_session("factura_sort3", "");
    $sort4 = art_session("factura_sort4", "");
    $sort5 = art_session("factura_sort5", "");
    $sort6 = art_session("factura_sort6", "");
    $sort7 = art_session("factura_sort7", "");
    $sort8 = art_session("factura_sort8", "");
    ...
    for ($i=1; $i<=8; $i++){
        $sorting = "";
        $sort_order = "";
        if (art_session("factura_sort".$i, "") == "1"){
            $sorting = "&factura_sort". $i . "=" . "2";
            $sort_order = "ASC";
        } else if (art_session("factura_sort".$i, "") == "2"){
            $sorting = "&factura_sort". $i . "=" . "1";
            $sort_order = "DESC";
        } else {
            $sorting = "&factura_sort". $i . "=" . "1";
            $sort_order = "";
        }
        $qrystr[$i] = "factura_ajax.php?clr_factura=t".$sorting;
    ...
    $advsearch_sql = art_session("factura_search","");
    if ($advsearch_sql != "") {
    ...
    art_sitemenu_display("factura", "defaulttheme");
    print "</td>\n";
    ...
    print "<a href=\"" . art_gen_url("factura_ajax.php?clr_factura_adv_session=y", 1). "\"" . " title=\"Show All\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/show_all.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . art_gen_url("factura_ajax.php?clr_factura=t", 1). "\"" . " title=\"Clear Sort\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/clear_sort.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . "./factura_search.php". "\"" . " title=\"Advance Search\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/adv_search.gif\" border=\"0\" /></a>";
    print "&nbsp;<a href=\"" . "./factura_add.php". "\"" . " title=\"Add New Record\" class=\"gridToolBarLink\"><img src=\"./images/defaultbutton/add.gif\" border=\"0\" /></a>";
    print "&nbsp;";
    print "                                </td>\n";
    print "                            </tr>\n";
    print "                        </table>\n";
    ...
    print "                    <td class=\"gridColumn\"  NOWRAP >\n";
    print "                        <div class=\"gridColumnText\">\n";
    print "                            <a href=\"" . art_gen_url($qrystr[2], 1) . "\" class=\"gridColumnLink\">Valor</a>\n";
    if ($sort2 == "1") {
    ...
    print "                    <td class=\"gridColumn\"  NOWRAP >\n";
    print "                        <div class=\"gridColumnText\">\n";
    print "                            <a href=\"" . art_gen_url($qrystr[3], 1) . "\" class=\"gridColumnLink\">Fecha Creacion</a>\n";
    if ($sort3 == "1") {
    ...
    print "                    <td class=\"gridColumn\"  NOWRAP >\n";
    print "                        <div class=\"gridColumnText\">\n";
    print "                            <a href=\"" . art_gen_url($qrystr[4], 1) . "\" class=\"gridColumnLink\">Temporal Id</a>\n";
    if ($sort4 == "1") {
    ...
    print "                    <td class=\"gridColumn\"  NOWRAP >\n";
    print "                        <div class=\"gridColumnText\">\n";
    print "                            <a href=\"" . art_gen_url($qrystr[5], 1) . "\" class=\"gridColumnLink\">Clientes Id</a>\n";
    if ($sort5 == "1") {
    ...
    print "                    <td class=\"gridColumn\"  NOWRAP >\n";
    print "                        <div class=\"gridColumnText\">\n";
    print "                            <a href=\"" . art_gen_url($qrystr[6], 1) . "\" class=\"gridColumnLink\">Usuario Id</a>\n";
    if ($sort6 == "1") {
    ...
    if ( ($numrows > 0) && ($result) ) {
        ...
        while ($row = mysql_fetch_array($result)){ 
            ...
            print "</td>\n";
            print "<td align=\"right\" >";
            $svalue = art_check_null( number_format( art_rowdata($row, 1) , 2, '.', ',' ) );
            if ($svalue != "&nbsp;"){
                $svalue = htmlspecialchars($svalue);
            }
            print $svalue;
            print "</td>\n";
            print "<td align=\"left\" >";
            $svalue = art_check_null( art_format_date( art_rowdata($row, 2) , "m/d/Y") );
            if ($svalue != "&nbsp;"){
                $svalue = htmlspecialchars($svalue);
            }
            print $svalue;
            ...
            print "<a href=\"" . "./factura_edit.php". "?pm_clientes_id=" . art_rowdata_byname($row, $field_names, "factura.clientes_id"). "&pm_id=" . art_rowdata_byname($row, $field_names, "factura.id"). "&pm_temporal_id=" . art_rowdata_byname($row, $field_names, "factura.temporal_id"). "&pm_usuario_id=" . art_rowdata_byname($row, $field_names, "factura.usuario_id"). "\"" . " title=\"Edit\" target=\"_self\" class=\"gridBodyLink\">";
            ...
            print "</td>\n";
            print "<td align=\"center\" >";
            print "<a href=\"" . "./factura_delete.php". "?pm_clientes_id=" . art_rowdata_byname($row, $field_names, "factura.clientes_id"). "&pm_id=" . art_rowdata_byname($row, $field_names, "factura.id"). "&pm_temporal_id=" . art_rowdata_byname($row, $field_names, "factura.temporal_id"). "&pm_usuario_id=" . art_rowdata_byname($row, $field_names, "factura.usuario_id"). "\"" . " title=\"Delete\" target=\"_self\" class=\"gridBodyLink\">";
    ...
    if ( ($numrows > 0) && ($result) ){
        ...
        if ($navtype != ""){
            $pagename = "factura";
            art_show_navigator($pagename, $navtype, $numrows, $page_size, $current_page, "factura_ajax.php", 1, 0, $use_ajax, $folder_button_images);
        }
    } else {
...
function del_selected_items($pagerecords) {
    ...
    for ($i=1; $i <= $pagerecords; $i++){
        ...
        if ($del_id != ""){
            ...
            $artv_pm_temporal_id = art_request("pm_temporal_id" . $i, "");
            $artv_pm_usuario_id = art_request("pm_usuario_id" . $i, "");
            if ($j==0){

                $sqlDel = "DELETE FROM `factura` WHERE ";
                $sqlDel .= "factura.clientes_id = " .  art_quote_intval($artv_pm_clientes_id);
                $sqlDel .= " AND factura.id = " .  art_quote_intval($artv_pm_id);
                $sqlDel .= " AND factura.temporal_id = " .  art_quote_intval($artv_pm_temporal_id);
                $sqlDel .= " AND factura.usuario_id = " .  art_quote_intval($artv_pm_usuario_id);
...
function art_groupdatagrid_display($field_names, $page_size, $current_page, $quick_search, $navtype, $category, $showtotalrec=0, $showpagesize=0) {	
    ...
    print "                    <td class=\"gridColumn\" ><div class=\"gridColumnText\">Valor</div></td>\n";
    print "                    <td class=\"gridColumn\" ><div class=\"gridColumnText\">Fecha Creacion</div></td>\n";
    print "                    <td class=\"gridColumn\" ><div class=\"gridColumnText\">Temporal Id</div></td>\n";

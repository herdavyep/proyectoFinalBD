...
function art_form_updatedata_display($form, $message){
    ...
    if (($aj != "1") && ($message == "")){
        ...
        $artv_fecha_creacion =  art_rowdata($row, 3) ;
    ...
    art_sitemenu_display("clientes_edit", "defaulttheme");
    print "</td>\n";
...
function art_update_data(){
    ...
    if (($result == "") && (art_session('art_form_loaded', 0) != 1)){
        ...
        $sql .= " clientes.nombre = " .  art_quote_strval($artv_nombre); 
        $sql .= ",clientes.cedula = " .  art_quote_strval($artv_cedula); 
        $sql .= ",clientes.fecha_creacion = " .  art_quote_dateval($artv_fecha_creacion); 
        $sql .= " WHERE ";
        $sql .= " clientes.id = " .  art_quote_intval($artv_pm_id);
    }

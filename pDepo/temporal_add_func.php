...
function art_form_insertdata_display($form, $message){
    ...
    art_sitemenu_display("temporal_add", "defaulttheme");
    print "</td>\n";
...
function art_insert_data() {
    ...
    if (($result == "") && (art_session('art_form_loaded', 0) != 1)){
        ...
        $sql .= " (";
        $sql .= "temporal.id"; 
        $sql .= ",temporal.compra_id"; 
        $sql .= ",temporal.fecha_creacion"; 
        $sql .= ") ";
        $sql .= " VALUES "; 
        $sql .= " (";
        $sql .=  art_quote_intval($artv_id); 
        $sql .= " , " .  art_quote_intval($artv_compra_id); 

...
function art_form_updatedata_display($form, $message){
    ...
    $gridtitle = "Calidad Edit";

    $aj = art_request("aj", ""); 
    if (($aj != "1") && ($message == "")){
        $sql = "select * from `calidad`";
        $sql .= " WHERE ";
        $sql .= " calidad.id = " .  art_quote_intval($artv_pm_id);
    ...
    art_sitemenu_display("calidad_edit", "defaulttheme");
    print "</td>\n";
...
function art_update_data(){
    ...
    if (($result == "") && (art_session('art_form_loaded', 0) != 1)){
        $sql = " UPDATE `calidad` SET ";
        $sql .= " calidad.nombre = " .  art_quote_strval($artv_nombre); 
        $sql .= ",calidad.fecha_creacion = " .  art_quote_dateval($artv_fecha_creacion); 
        $sql .= " WHERE ";
        $sql .= " calidad.id = " .  art_quote_intval($artv_pm_id);
    }

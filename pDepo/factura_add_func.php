...
function art_form_insertdata_display($form, $message){
    ...
    art_sitemenu_display("factura_add", "defaulttheme");
    print "</td>\n";
...
function art_insert_data() {
    ...
    if (($result == "") && (art_session('art_form_loaded', 0) != 1)){
        ...
        $sql .= "factura.id"; 
        $sql .= ",factura.valor"; 
        $sql .= ",factura.fecha_creacion"; 
        $sql .= ",factura.temporal_id"; 
        $sql .= ",factura.clientes_id"; 
        $sql .= ",factura.usuario_id"; 
        $sql .= ") ";
        $sql .= " VALUES "; 
        $sql .= " (";
        $sql .=  art_quote_intval($artv_id); 
        $sql .= " , " .  art_quote_intval($artv_valor); 
        $sql .= " , " .  art_quote_dateval($artv_fecha_creacion); 
        $sql .= " , " .  art_quote_intval($artv_temporal_id); 
        $sql .= " , " .  art_quote_intval($artv_clientes_id); 
        $sql .= " , " .  art_quote_intval($artv_usuario_id); 
       $sql .= ") ";
    }
